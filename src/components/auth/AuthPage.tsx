...
interface AuthPageProps {
  ...
  onRegister: (username: string, phone: string, password: string) => string | null;
}
...
export function AuthPage({ onLogin, onRegister }: AuthPageProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      const err = onRegister(username.trim(), phone.trim(), password);
      if (err) setError(err);
    } else {
    ...
    <div className="min-h-screen bg-brand-bg flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full py-3.5 bg-amber-500 hover:bg-amber-400 active:scale-95 text-black font-black text-sm uppercase tracking-widest rounded-xl transition-all shadow-[0_0_30px_rgba(245,158,11,0.2)]"
          >
