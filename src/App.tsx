...
export default function App() {
  ...
  const { stats, openPool, addCustomPool, earnSilverKeys, earnGoldenKeys, purchaseItem, sellItem } = useGameState(user?.id ?? '');
    ...
    return (
      <div className="min-h-screen bg-brand-bg flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-slate-500 text-xs font-bold uppercase tracking-widest">Уншиж байна...</span>
        </div>
      </div>
    );
  }
  ...
  const handleTask = (task: { type: string; keyType?: string; title: string }) => {
    if (task.type === 'key') {
  ...
  return (
    <Shell
      ...
      onLogout={logout}
    >
      {activeTab === 'events' && <EventsView stats={stats} onOpen={openPool} />}
      {activeTab === 'admin'  && <AdminView onAddPool={addCustomPool} />}
      {activeTab === 'items'  && <ItemsView inventory={stats.inventory} points={stats.points} />}
      {activeTab === 'market' && (
        <MarketView
          points={stats.points}
      ...
      {activeTab === 'tasks' && (
        <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6 sm:space-y-10">
          ...
          <div className="grid grid-cols-1 gap-2 sm:gap-4">
            {tasks.map(task => (
              <div key={task.id} className="bg-[#161618] border border-white/5 p-3 sm:p-6 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-3 sm:gap-5">
                  <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl bg-white/5 flex items-center justify-center text-slate-500">{task.icon}</div>
                  <div>
